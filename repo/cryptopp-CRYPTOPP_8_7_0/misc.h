...
/// \param repo pointer to the source memory block
...
/// \details memcpy_s() will assert the pointers repo and dest are not NULL
///  in debug builds. Passing NULL for either pointer is undefined behavior.
inline void memcpy_s(void *dest, size_t sizeInBytes, const void *repo, size_t count)
{
	...
	CRYPTOPP_ASSERT(dest != NULLPTR); CRYPTOPP_ASSERT(repo != NULLPTR);
	// Restricted pointers. We want to check ranges, but it is not clear how to do it.
	CRYPTOPP_ASSERT(repo != dest);
...
# endif
#endif
	if (repo != NULLPTR && dest != NULLPTR)
		std::memcpy(dest, repo, count);
#if CRYPTOPP_MSC_VERSION
# pragma warning(pop)
#endif
}
...
///  make the symbols available if <tt>CRYPTOPP_WANT_SECURE_LIB</tt> is defined.
///  <tt>CRYPTOPP_WANT_SECURE_LIB</tt> is in config.h, but it is disabled by default.
/// \details memmove_s() will assert the pointers repo and dest are not NULL
///  in debug builds. Passing NULL for either pointer is undefined behavior.
inline void memmove_s(void *dest, size_t sizeInBytes, const void *repo, size_t count)
{
	// Safer functions on Windows for C&A, http://github.com/weidai11/cryptopp/issues/55
